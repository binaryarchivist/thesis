...
class DocumentVersionDetailAPIView(APIView):
    ...
    @swagger_auto_schema(
        ...
        #     properties={
        #         "file": openapi.Schema(
        #             type=openapi.TYPE_FILE, description="PDF or image"
        #         )
        #     },
        # ),
        responses={
    ...
    def post(self, request, document_id):
        ...
        if not upload:
            return Response(
                {"error": "A file is required."}, status=status.HTTP_400_BAD_REQUEST
            )

        # 3) Create and save the new version (model.save() auto-assigns version_number)
        version = DocumentVersion(document=document, file=upload, created_by=user)
        version.save()
